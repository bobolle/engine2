...
int main(void) {
  ...
  game_window.set_resize_callback(
    [&](float aspect) { camera_noclip.set_aspect(aspect); }
  );
  ...
  while(!glfwWindowShouldClose(window)) {
    ...
    camera_noclip.update(dt);
    view       = camera_noclip.get_view();
    projection = camera_noclip.get_projection();
